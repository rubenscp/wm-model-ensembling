...
from model_detr.dataset import *
...
def inference_detr_model_with_dataset_test(parameters, device, model, processor, 
    ...
    threshold = parameters['model_detr']['neural_network_model']['threshold']
    ...
    inference_metric = Metrics(
        model=parameters['model_detr']['neural_network_model']['model_name'],
        # number of classes doesn't consider the background class
        number_of_classes=parameters['model_detr']['neural_network_model']['number_of_classes'] - 1,
    )
    ...
    model_name = parameters['model_detr']['neural_network_model']['model_name']
    num_classes = parameters['model_detr']['neural_network_model']['number_of_classes']
    threshold = parameters['model_detr']['neural_network_model']['threshold']
    iou_threshold = parameters['model_detr']['neural_network_model']['iou_threshold']
    ...
    title += LINE_FEED + \
             'Confidence threshold: ' + str(parameters['model_detr']['neural_network_model']['threshold']) + \
             '   IoU threshold: ' + str(parameters['model_detr']['neural_network_model']['iou_threshold']) + \
             '   Non-maximum Supression: ' + str(parameters['model_detr']['neural_network_model']['non_maximum_suppression'])
    path_and_filename = os.path.join(
        parameters['test_results']['metrics_folder'], 
        parameters['model_detr']['neural_network_model']['model_name'] + \
    ...
    print(f'parameters')
    print(f'inference_metric.full_confusion_matrix: {inference_metric.full_confusion_matrix}')
    print(f'path_and_filename: {path_and_filename}')
    print(f'x_labels_names: {x_labels_names}')
    print(f'y_labels_names: {y_labels_names}')
    print(f'path_and_filename: {path_and_filename}')
    print(f'inference_metric.tp_per_class: {inference_metric.tp_per_class}')
    print(f'inference_metric.fp_per_class: {inference_metric.fp_per_class}')
    print(f'inference_metric.fn_per_class: {inference_metric.fn_per_class}')
    print(f'inference_metric.tn_per_class: {inference_metric.tn_per_class}')
    ...
    sheet_list.append(['', ''])
    sheet_list.append(['Threshold',  f"{parameters['model_detr']['neural_network_model']['threshold']:.2f}"])
    sheet_list.append(['IoU Threshold',  f"{parameters['model_detr']['neural_network_model']['iou_threshold']:.2f}"])
    sheet_list.append(['Non-Maximum Supression',  f"{parameters['model_detr']['neural_network_model']['non_maximum_suppression']:.2f}"])
    sheet_list.append(['', ''])
