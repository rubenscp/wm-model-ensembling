"""
Implements the test images inference of the trained DETR model.
"""
...
from model_detr.model import *
from model_detr.dataset import *
from model_detr.inference import * 
...
def model_detr_inference(parameters, processing_tasks, device):    

    logging_info('')
    logging_info('-----------------------------------')
    logging_info(f'Inferencing DETR model')
    logging_info('-----------------------------------')
    logging_info('')    
    ...
    parameters['test_results']['inferenced_image_folder'] = os.path.join(
        parameters['test_results']['inferenced_image_folder'], 'model_detr'
    )
    parameters['test_results']['inferenced_image_folder'] = os.path.join(
    ...
    all_predictions = inference_detr_model_with_dataset_test(parameters, device, model, 
                      processor, dataset_test, dataset_test_original_boxes)
...
def set_input_image_folders(parameters):
    ...
    image_dataset_folder = os.path.join(
        ...
        parameters['model_detr']['input']['input_dataset']['input_dataset_path'],
        parameters['model_detr']['input']['input_dataset']['annotation_format'],
...
def copy_weights_file(parameters):
    ...
    logging_info(f"Folder name: {parameters['model_detr']['input']['inference']['weights_folder']}")
    logging_info(f"Filename   : {parameters['model_detr']['input']['inference']['weights_filename']}")

    Utils.copy_file_same_name(
        parameters['model_detr']['input']['inference']['weights_filename'],
        parameters['model_detr']['input']['inference']['weights_folder'],
...
def get_dataset(parameters, device, dataset_type):
    ...
    pretrained_model_name_or_path = parameters['model_detr']['neural_network_model']['pretrained_model_path']
    cache_dir = parameters['model_detr']['neural_network_model']['model_cache_dir']
...
def get_neural_network_model(parameters, device):
    ...
    learning_rate = parameters['model_detr']['neural_network_model']['learning_rate_initial']
    learning_rate_backbone = parameters['model_detr']['neural_network_model']['learning_rate_backbone']
    weight_decay = parameters['model_detr']['neural_network_model']['weight_decay']
    num_labels = parameters['model_detr']['neural_network_model']['number_of_classes']
    pretrained_model_name_or_path = parameters['model_detr']['neural_network_model']['pretrained_model_path']
    cache_dir = parameters['model_detr']['neural_network_model']['model_cache_dir']
